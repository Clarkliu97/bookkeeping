...
Administrative posted-journal changes remain controlled. Locked-period entries must be unlocked first; reconciled journals must be unmatched; and generated system and depreciation journals stay under their owning workflows. An actively reversed journal must be un-reversed before it can be edited or deleted. Once restored to `posted`, the original can be corrected even though its prior reversal remains `voided` for history. A later permanent deletion removes the original and all of its voided reversals together in one transaction, with an audit snapshot for every removed entry. Posted edits preserve the original posting identity and timestamp. In the update-journal popup, superusers receive an explicit **Update posted journal** button; successful posted updates and deletions also reload the ledger with its current date and account filters so previously loaded results do not remain stale. Every administrative operation is unavailable to ordinary preparers, reviewers, and approvers.

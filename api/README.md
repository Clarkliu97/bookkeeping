...
- `single`: exactly one source file and exactly one recommended journal
- `multiple`: from one to 50 source files and from one to 50 recommended journals

The create request is multipart form data with repeated `files` fields plus `analysis_mode`, `model`, and optional operator context or target-period fields. Source files receive stable numbers in upload order. The structured result groups those numbers per recommended journal, so several documents can support the same economic transaction while unrelated transactions remain separate.

Document assignments are many-to-many. A source such as a monthly bank statement can be assigned to every journal containing a transaction evidenced by that statement. When a multiple-mode run contains more than one file, an active `accrual` reporting basis also adds a conditional timing instruction: when visible clearance evidence is at least five calendar days after the invoice date, prefer separate invoice-recognition and liability-clearance journals when the evidence supports that treatment. Cash-basis and one-file runs do not receive this instruction.

Before a run becomes review-ready, the service verifies that every uploaded document is assigned, recommendation and line numbers are sequential, every line is one-sided, and every recommended journal balances independently. Accepting a batch returns `{ "journals": [...] }`, creates all drafts in one transaction, and links each draft only to the documents assigned to it. Nothing is posted automatically.

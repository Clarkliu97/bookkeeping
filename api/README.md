...
`PUT /companies/{company_id}/journals/{journal_id}` continues to update drafts and additionally lets superusers correct eligible posted journals. Posted changes preserve the journal ID, entry number, `posted_at`, and `posted_by_user_id`, revalidate the complete balanced entry, and record `journal.posted_updated` with before and after snapshots. Both the current and destination periods must be unlocked. Reconciled, generated, posted-reversal, and actively reversed entries cannot be edited in place. An original restored through `unreverse` can be edited while its prior reversal remains `voided`. The operator popup labels this action **Update posted journal** and reloads the general-ledger report with its current filters after the update succeeds.

`DELETE /companies/{company_id}/journals/{journal_id}` continues to let preparers delete drafts and additionally lets superusers permanently delete eligible posted journals. A posted entry cannot be deleted from a locked period, while reconciled, system-generated, depreciation, posted-reversal, and actively reversed entries must be resolved through their owning workflows. When a restored original has one or more voided reversals, the endpoint validates every entry first and then deletes the original and all voided reversal rows atomically. It records `journal.posted_deleted` for the original and `journal.voided_reversal_deleted` with a full snapshot for each associated reversal before removing their lines, direct evidence links, and journal-recommendation pointers. The popup reports how many voided reversals will be removed and reloads the current general-ledger view after deletion.
...
Bank-account management exposes separate create, update, and delete operations. `POST /api/companies/{company_id}/bank-accounts` creates an active account, `PUT /api/companies/{company_id}/bank-accounts/{bank_account_id}` updates its metadata, and `DELETE /api/companies/{company_id}/bank-accounts/{bank_account_id}` soft-deletes it by marking it inactive. Create and update accept an optional `ledger_account_id`; it must identify an active, company-owned asset or liability account. This link is required for grouped reconciliation and identifies the journal line movement that represents the bank account.
...
When a reconciliation session has an accounting period, its statement items are limited to bank-row transaction dates within the period and journal candidates must be posted, assigned to that same period, and dated within its boundaries. The API enforces this scope when matching, not only in the operator interface. Changing the period re-scopes unresolved items and is blocked after any item has been matched or ignored. Sessions created without a period retain the unrestricted workflow.

Grouped reconciliation is available through `GET|POST /api/companies/{company_id}/reconciliation-sessions/{session_id}/match-groups` and `DELETE /api/companies/{company_id}/reconciliation-sessions/{session_id}/match-groups/{group_id}`. A group contains one or more signed statement allocations and one or more signed journal allocations, so the same contract supports n-to-1, 1-to-n, and n-to-n relationships. Journal source amounts are calculated only from lines on the bank account's linked ledger account (`debit - credit`); statement source amounts use `credit - debit`.

Allocation amounts default to the source's remaining unallocated balance and may be reduced for partial settlements. Each allocation must retain its source's sign, but a group may contain both positive and negative sources so net payouts, refunds, fees, chargebacks, and similar settlements can be reconciled without inventing separate matches. The service rejects zero or opposite-direction allocations, over-allocation (including allocation through another reconciliation item for the same imported row), mixed currencies, non-posted journals, and period violations. The signed net totals must reconcile: a difference larger than `tolerance_amount` is rejected, and any accepted non-zero difference requires `note`. Partially allocated statement items stay unmatched and staged, while fully allocated items become matched. Confirmed staged rows already belonging to an open session are not copied into another session. Deleting a group recalculates the affected statement states and records an audit event; completed sessions cannot be changed.

`POST /api/companies/{company_id}/reconciliation-sessions/{session_id}/auto-reconcile` attempts the open queue using `amount_tolerance` (default `0.01`, maximum `10.00`), `date_window_days` (default `3`, maximum `31`), and `max_group_size` per side (default `3`, maximum `4`). It first searches every possible 1-to-1 amount/date pair, then performs a deterministic bounded local subset search for 1-to-n, n-to-1, and n-to-n groups. The linked ledger account's remaining signed movement is the authoritative journal amount. Absolute amounts from all journal detail lines are used only as a tie-breaker, allowing a multi-line journal to rank ahead of an aggregate-only alternative when its components explain the statement rows.

Candidate quality is ordered by amount difference, source count, date proximity, detailed-line support, and a stable identifier order. If a statement item has multiple different candidates with the same best quality, it is reported in `ambiguous_statement_item_ids` and left unmatched. Sources with no amount match inside the date window also remain open. Zero-net groups are not created automatically. Selected groups are non-overlapping and are committed atomically; a concurrent validation failure rolls the whole auto-reconcile operation back. The response includes considered and matched item counts, created groups, and all remaining item IDs. Every created group and the overall run are audited. The bounded grouped search limits local source pools to keep dense sessions responsive, so high-cardinality or unusually dense settlements may still require manual grouping.
